...
# 数据集路径
path = r"D:\Code\python\Mask_RCNN-master\samples\my_data\script\test\\"
# 生成数据的保存路径
save_path = r"D:\下载\IDM下载\压缩文件\archive2\\"
# 当前数据集图片格式/
file_format = ".png"
# file_format1 = ".jpg"
# 替换格式jpg -> json
...
for i in range(len(img_list)):
    ...
    if is_exists:
        ...
        # 修改json文件名
        setting['imagePath'] = img_name.replace('m','ml')
        full_path = save_path + img_name.replace('m','ml')
...
for i in range(len(img_list)):
    ...
    if is_exists:
        ...
        # file_name = setting['imagePath'].split('\\')[-1]
        img_name = img_path.split("\\")[-1]
        setting['imagePath'] = img_name.replace('m','mu')
        # setting['imagePath'] = TB + file_name
        full_path = save_path + img_name.replace('m','mu')
